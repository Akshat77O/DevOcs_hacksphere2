...
import { listenToQueueUpdates, stopListening } from "../services/queueService";
import { api } from "../services/api";
import { toast } from "sonner";
import { updateUserQueueStatus } from "../services/queueService";
...
const QueueManagement = () => {
    const [activeTab, setActiveTab] = useState<"current" | "departments">(
        "current"
    );
    ...
    useEffect(() => {
        listenToQueueUpdates((updatedQueue) => setQueueItems(updatedQueue));
    ...
    //         const formattedQueueItems = Array.isArray(response)
    ...
    const handleStatusChange = async (id: string, newStatus: string) => {
        try {
            await updateUserQueueStatus(id, newStatus);
                ...
                <>
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-8 bg-white rounded-lg border border-gray-200">
                            ...
                            <p className="text-gray-500">
                                There are no patients waiting at the moment
                            </p>
                        </div>
                    )}
                </>
                        ...
                        >
                            <div className="flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">
                                        Capacity:{" "}
                                        {Math.round(dept.capacity * 100)}%
                                    </p>
                                </div>
                                <div className="text-right">
                                    <div className="font-bold text-lg text-hospital-600">
                                ...
                                <div className="w-full h-2 bg-gray-200 rounded-full">
                                    <div
                                        className={`h-2 rounded-full ${
                                            dept.waitTimeMinutes > 30
                                                ? "bg-red-500"
                                                : dept.waitTimeMinutes > 15
                                                ? "bg-amber-500"
                                                : "bg-green-500"
                                        }`}
                                        style={{
